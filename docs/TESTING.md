...
**Current suite: 51 tests, all passing, across 7 files:**

- `AnchorTests/StreakServiceTests.swift` — the largest by far (25 cases). Streak continuity, best-vs-current streak, weekly-target streaks, `dailyHistory`'s multi-way state (`.completed`/`.partial`/`.missed`/`.notDue`/`.shielded`), shield streak-continuity/non-increment/priority-ordering, quantifiable-habit threshold breaking/completing/history-transition, and (as of Calendar History, v1.1) `dayCompletionState`'s single-day lookups — cross-checked against `dailyHistory` for consistency, future-date and pre-creation-date `.notDue`, and today-with-nothing-logged `.missed` without streak leniency.
...
- Calendar History and the Today-screen interaction polish (full-card tap, swipe-to-delete, 100%-completion celebration) were blocked in this tool by `HabitsView`'s "+" button not responding to synthetic taps (2026-07-31) — verified working end-to-end by the user directly instead. See `KNOWN_ISSUES.md` R7.
...
- Coordinate space for taps is in **device points**, not screenshot pixels — always use the `(width, height)` reported by the `attach` call, not pixel measurements eyeballed from a screenshot image. **Confirmed 2026-07-30 as a real, repeatable failure mode:** tapping pixel-scale coordinates directly (e.g. treating a ~918×1996px screenshot as if it were the 402×874pt coordinate space) causes taps to silently miss or land out of bounds, which looks identical to "taps stopped registering app-wide." Once coordinates were correctly scaled by the ratio between the screenshot and the `attach` call's reported point dimensions, taps/swipes/long-presses were reliable for the rest of that session. Worth ruling this out before concluding the environment itself is flaky.
- **Confirmed 2026-07-31:** a single `ToolbarItem`-hosted `Button` (`HabitsView`'s "+" Add Habit action) can stop responding to synthetic taps while every other control in the same session — tab bar items, in-view `Button`s — works normally. Ruled out: wrong coordinates (cross-checked against known-good tab-bar positions), a stuck gesture recognizer (full app restart), and a stale simulator boot (fresh boot, same result). Not diagnosed; worked around by the user testing directly on their own device instead. See `KNOWN_ISSUES.md` R7. If this recurs, don't assume it's app-wide unresponsiveness — it may be isolated to one specific `ToolbarItem`/UIKit-bridged control the way `UIColorWell` and the on-screen keyboard were.
