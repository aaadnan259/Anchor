...
**Components/** (16 files): the reusable building blocks — `HabitCardView` (the Today/Habits list row, with expandable/quantifiable/binary/shielded branches, whole card a `Button` mirroring its primary control), `HabitIconView` (renders SF Symbol or emoji from the same `icon: String`), `IconPickerView`, `AccentColorPickerView`, `ProgressRingView` (has an opt-in `pulseGlow` param used only by Today's header ring), `CompletionToggleView`, `CompletionCelebrationView` (the 0→1 daily-progress celebration burst), `HabitHistoryGridView` (the dot-matrix heatmap), `CalendarHistoryGridView` (the v1.1 month-grid calendar — distinct presentation of the same `DayCompletionState` data), `TrendChartView`, `TimeOfDayChartView`, `WeeklyBarsView`, `OccurrenceRowView`, `PresetCardView`, `PrimaryButtonView`, `SectionHeaderView`.
...
None — every feature task reached completion, including v1.1. This documentation pass is the only open work as of this writing.
...
See `PROJECT_STATE.md` for the live version — it's updated every session and this section is not. As of this writing (2026-07-31): the full v1 feature set plus v1.1 (Calendar History) is shipped and user-confirmed working; v1.2 (Widgets) was scoped and researched but is explicitly paused pending a paid Apple Developer account, at the user's request — do not raise it again until they say they're ready. `docs/TODO.md` and `docs/KNOWN_ISSUES.md` both have nothing open. The one open item worth remembering for a future session: `HabitsView`'s "+" button not responding to synthetic taps in this environment's simulator-automation tool (`KNOWN_ISSUES.md` R7) was worked around via direct user testing, not diagnosed — it may recur.
