...
- **Open / unverified:** none, as of 2026-07-30. Every item ever logged in `KNOWN_ISSUES.md` — shield long-press, heatmap richness, and (on a physical device) emoji entry and the custom `ColorPicker` — has been verified working or fixed. See `KNOWN_ISSUES.md`'s Resolved section (R4–R6) for detail.

---

## Development History

Reconstructed from `git log` (17 commits, dated 2026-07-23 through 2026-07-30) plus session narrative. Full changelog in `CHANGELOG.md`.
...
See `PROJECT_STATE.md` for the live version — it's updated every session and this section is not. As of this writing (2026-07-30): every application-code feature task is complete, the documentation has been audited against the real codebase (not just generated once and trusted), and every item ever logged in `docs/TODO.md` or `docs/KNOWN_ISSUES.md` has been closed — either fixed in code (unit label cap, heatmap partial-state reuse) or verified working end-to-end (shield long-press in the simulator; emoji entry and the custom `ColorPicker` on a physical device). PR #5 (the physical-device verification writeup) may still be open pending merge — check `git log origin/main` rather than assuming.
