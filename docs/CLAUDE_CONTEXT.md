...
**Current development stage.** Feature-complete for the v1 scope defined in `PROJECT_SPEC.md`, plus v1.1 (Calendar History) shipped and user-confirmed working. v1.2 (Widgets) was scoped, researched, and then explicitly paused by the user pending a paid Apple Developer Program account (not attempted — see [Development History](#development-history) and `DECISIONS.md` ADR-017). Do not raise Widgets again until the user says they're ready.
...
Anchor/
  ...
  Extensions/     Color+Hex, String+Icon, DayCompletionState+Color
...
**Views/** (12 files): `TodayView`, `HabitsView`, `StatsView`, `AddEditHabitView`, `HabitInsightsView`, `SettingsView`, `OnboardingView`, `RootTabView`, `LockScreenView`, `LogValueView`, `ManageShieldsView`, `CalendarHistoryView` (v1.1 — pushed from `HabitInsightsView`'s History section).

**Components/** (17 files): the reusable building blocks — `HabitCardView` (the Today/Habits list row, with expandable/quantifiable/binary/shielded branches, whole card a `Button` mirroring its primary control), `HabitIconView` (renders SF Symbol or emoji from the same `icon: String`), `IconPickerView`, `AccentColorPickerView`, `ProgressRingView` (has an opt-in `pulseGlow` param used only by Today's header ring), `CompletionToggleView`, `CompletionCelebrationView` (the 0→1 daily-progress celebration burst), `HabitHistoryGridView` (the dot-matrix heatmap), `CalendarHistoryGridView` (the v1.1 month-grid calendar — distinct presentation of the same `DayCompletionState` data), `TrendChartView`, `TimeOfDayChartView`, `WeekdayRadialChartView` (the app's only genuinely radial chart — a weekday completion wheel via Swift Charts' `SectorMark`), `WeeklyBarsView`, `OccurrenceRowView`, `PresetCardView`, `PrimaryButtonView`, `SectionHeaderView`.
...
**Extensions/** (3 files): `Color+Hex`, `String+Icon`, `DayCompletionState+Color` (the `color(tint:)` mapping shared by `HabitHistoryGridView` and `CalendarHistoryGridView`, factored out during Calendar History to avoid duplicating it between the two).

**AnchorTests/** (8 files): `StreakServiceTests.swift` is the largest and most important (25 cases — streaks, shields, quantifiable thresholds, daily history, single-day `dayCompletionState` lookups). `InsightsServiceTests`, `ScheduleProviderTests`, `ExportServiceTests`, `ColorHexTests`, `StringIconTests` cover their respective services/extensions. `AnchorTests.swift` — despite the generic Xcode-scaffold name — actually contains the `DueDateRule` suite. `TestSupport.swift` holds shared fixtures (a fixed UTC `Calendar` for date-math determinism, an in-memory `ModelContext` factory).
...
| Today screen interaction and motion polish | Completion celebration burst at 100% daily progress, pulsing glow on the header progress ring, tap-anywhere-on-card completion, swipe-to-delete on Today, removed the Habits tab's redundant `EditButton()` | `CompletionCelebrationView`, `ProgressRingView.pulseGlow`, `HabitCardView`, `TodayViewModel.delete` |
| Calendar History (v1.1) | Dedicated month-grid calendar of a habit's completion history, pushed from Habit Insights' History section, distinct from the existing dot-matrix heatmap | `StreakService.dayCompletionState`, `CalendarHistoryGridView`, `CalendarHistoryView` |
| "By Day of Week" radial chart | Closes the backlog's "additional chart types" item — a radial wheel (Swift Charts `SectorMark`), one wedge per weekday, wedge length = that weekday's historical completion rate; a third Habit Insights section alongside Trend and Time of Day | `InsightsService.weekdayDistribution`, `WeekdayRadialChartView` |

### In Progress

None — every feature task reached completion, including v1.1 and the radial-chart backlog item. Simulator visual verification of the radial chart is the only outstanding item as of this writing (see `TODO.md`).

### Planned (v1.2+, not to be implemented until scheduled or explicitly requested)

- **v1.2 — Widgets.** Scoped and researched (2026-07-31): needs a new WidgetKit extension target sharing data via an App Group. **Explicitly paused by the user** — free "Personal Team" signing makes App Group provisioning unreliable and risks the existing physical-device build pipeline; revisit only once the user enrolls in a paid Apple Developer account and says they're ready. Do not raise this proactively. See `DECISIONS.md` ADR-017.
...
~~**Additional chart types**~~ — shipped 2026-07-31 as the "By Day of Week" radial chart. See ADR-018.
...
8. **WidgetKit (v1.2) paused rather than implemented, even though it was requested and scoped.** Sharing data with a widget needs an App Group, which risks the user's free-tier code signing (the same category of problem ADR-003 already caused once). The user chose to drop it entirely for the session rather than risk the physical-device build pipeline, pending a paid Apple Developer account. See ADR-017.
9. **"Additional chart types" interpreted as a weekday completion wheel**, not a bigger single-ring visualization or a radial re-skin of the existing time-of-day bars. Chosen for being both authentically radial and a genuinely new insight, not just a new shape on old data. See ADR-018.
...
- **Fixed 2026-07-30:** `StreakService.dailyHistory` reported `.missed` for a quantifiable habit logged below target, identical to a day with nothing logged at all. Fixed by reusing the existing `.partial` state (see ADR-015).
- **Fixed 2026-07-31 (caught before it shipped):** extracting `dailyHistory`'s per-day branch into a reusable helper for Calendar History would have silently made an unlogged *future* day report `.missed` (nothing was ever logged for a day that hasn't happened). Fixed with an explicit `day > today` guard, reusing `.notDue` (see ADR-016).
- **Open / unverified:** none, as of 2026-07-31. Every item ever logged in `KNOWN_ISSUES.md` — shield long-press, heatmap richness, emoji entry, the custom `ColorPicker`, and (2026-07-31) Calendar History plus the Today-screen interaction polish — has been verified working or fixed. The last of these was blocked in the simulator-automation tool by a new, specific gap (`HabitsView`'s "+" button not responding to synthetic taps) and closed out instead by the user testing directly on their own device — the tooling gap itself was never diagnosed and could resurface. See `KNOWN_ISSUES.md`'s Resolved section (R4–R7) for detail.

---

## Development History

Reconstructed from `git log` (28 commits, dated 2026-07-23 through 2026-07-31) plus session narrative. Full changelog in `CHANGELOG.md`.
...
14. **Documentation restructure** (`d9d77e4`, 2026-07-29) — generated this `docs/` knowledge base and moved `PROJECT_SPEC.md`/`ARCHITECTURE.md` into it (see ADR-014).
15. **Documentation audit** (`d293b88`, PR #1, 2026-07-30) — a fresh session was asked to verify the just-generated docs against the actual codebase rather than trust them. Found and fixed several inaccuracies introduced by the previous session: a "40+ cases" test-count claim for `StreakServiceTests.swift` (actually 20 at the time) repeated across three files; off-by-one `Models/`/`Services/` file counts; `Completion`/`Occurrence` fields documented as raw `habitID`/`occurrenceID` when they're actually SwiftData relationships; a missing `HabitService.duplicate`/`updateOccurrence` mention; and a `PROJECT_SPEC.md` self-contradiction (Export/Import and charts listed as both shipped and out-of-scope).
16. **Simulator verification pass** (`7857311`, PR #2, 2026-07-30) — attempted the three interaction paths `KNOWN_ISSUES.md` had logged as unverified (emoji entry, custom `ColorPicker`, shield long-press). Root-caused the *previous* session's "taps stopped registering app-wide" symptom: this simulator-control tool's tap coordinates are in **points** (402×874 on iPhone 16 Pro), not screenshot pixels — using pixel-scale coordinates directly silently misses controls. Once corrected, shield long-press verified working end-to-end (including confirming the Today quick-action and `ManageShieldsView` stay in sync). Emoji entry and the `ColorPicker` remained blocked, now for a precise, non-code reason: no on-screen software keyboard renders in this tool's simulator stream, and `UIColorWell` doesn't respond to synthetic taps even though identical coordinates correctly select a regular color swatch.
17. **Unit label length cap** (`f0e80b4`, PR #3, 2026-07-30) — closed a Low-priority `TODO.md` item: `AddEditHabitViewModel.unitLabel` now clamps to 24 characters via a `didSet`. Emoji-blocking (also mentioned in the original TODO wording) was deliberately skipped as unnecessary.
18. **Heatmap richness for below-target quantifiable days** (`5084e28`, PR #4, 2026-07-30) — closed the last `TODO.md` item that didn't need a physical device. `StreakService.dailyHistory` now reports `.partial` instead of `.missed` for a quantifiable habit logged below target, **reusing** the existing `.partial` state/color rather than adding a new `DayCompletionState` case — see ADR-015 for why that was simpler than originally anticipated, and why a shielded quantifiable day was never actually part of the gap.
19. **Physical-device verification** (`940333f`, PR #5, 2026-07-30) — built and installed the app on the developer's own iPhone again (free "Personal Team" signing, one-time developer-profile trust) specifically to close the last two `KNOWN_ISSUES.md` items that needed a real device or human tester. The user confirmed both emoji habit-icon entry and the custom `ColorPicker` work end-to-end with no crashes. `docs/TODO.md` and `docs/KNOWN_ISSUES.md` both have nothing open as of this entry. Discovered along the way that `xcrun xctrace list devices` and `xcrun devicectl list devices` report *different identifier strings for the same physical device* — passing the wrong one to the wrong tool produces a misleading "device not found" error.
20. **Handoff-consistency pass** (`34be51e`, PR #6, 2026-07-30) — brought `CLAUDE_CONTEXT.md`/`PROJECT_STATE.md` fully current before a new session picked up work, closing the loop on PR #5's pending-merge caveat above.
21. **Today screen interaction and motion polish** (`40ca2d3`, 2026-07-31) — five requested tweaks: a completion celebration burst when daily progress hits 100%, a pulsing glow on the header progress ring, tapping anywhere on a habit card to complete it (mirroring the card's existing primary control), swipe-to-delete on Today (required converting its `ScrollView`/`VStack` to a `List`), and removing the Habits tab's redundant `EditButton()` (drag-to-reorder already works without it). A sixth request — extend Shields to weekly-goal habits — conflicted with ADR-009's deliberate scoping; surfaced via `AskUserQuestion`, the user chose to leave it as-is. Committed directly to `main` at the user's explicit request (no PR branch).
22. **Calendar History (v1.1) shipped; WidgetKit (v1.2) researched and paused** (`a1f6146`, 2026-07-31) — the user asked for both features in one session. Used Plan Mode: direct code research plus an Explore agent and a Plan agent validation pass surfaced that sharing data with a widget needs an App Group, which risks the user's free "Personal Team" signing (the same category of risk ADR-003 already hit once). Surfaced via `AskUserQuestion` (a snapshot-vs-live-SwiftData-sharing design fork); the user then went further and explicitly dropped WidgetKit entirely for the session, and revised Calendar History's UX from a planned segmented-control toggle to a dedicated pushed screen. Shipped: `StreakService.dayCompletionState` (a single-day lookup extracted from `dailyHistory`'s per-day branch, with a `day > today` guard fixing a real bug — see the Bugs section above), `CalendarHistoryGridView`, `CalendarHistoryView`, and a shared `DayCompletionState.color(tint:)` extension so the new calendar and the existing heatmap don't each define their own color mapping. See `DECISIONS.md` ADR-016/ADR-017.
23. **Simulator-tooling gap logged, then resolved via direct user testing** (`89f5507` then `96d51d5`, 2026-07-31) — attempting to verify the above two features' UI, `HabitsView`'s "+" (Add Habit) toolbar button stopped responding to synthetic taps in the simulator-automation tool across 10+ varied attempts and a fresh simulator boot, while every other control tested worked normally — a more specific, isolated finding than earlier sessions' general "taps stop working" symptom. Logged as `KNOWN_ISSUES.md` #5. The user then confirmed, by testing directly on their own device, that Calendar History and the Today-screen polish all work correctly with no bugs — closing the verification need (`KNOWN_ISSUES.md` R7) without ever diagnosing the automation-tool gap itself, which could resurface in a future simulator-driven session.
24. **"By Day of Week" radial chart** (uncommitted as of this writing, 2026-07-31) — asked what was next, offered the two remaining backlog items (cross-habit correlation insights — flagged as blocked on real usage data — and additional chart types); the user picked chart types. "Radial completion charts" was only ever a placeholder phrase in the backlog, so a concrete design (a weekday completion wheel, reasoning in ADR-018) was proposed and confirmed before implementing. Shipped: `InsightsService.weekdayDistribution` + `WeekdaySlice` (reusing the same `DueDateRule.isDue`/`CompletionService.isFullyCompleted` choke points `dailyRate` already uses), `WeekdayRadialChartView` (Swift Charts `SectorMark` — hit one build error, `outerRadius` has no `PlottableValue`-bound overload in this SDK, fixed by computing the ratio directly per-mark), a new Habit Insights section, 3 new `InsightsServiceTests` cases. A `simctl install`/`launch` for simulator verification timed out mid-boot and the session was interrupted before it could be retried — logged as a fresh `TODO.md` Medium item rather than left unmentioned.

---

## Current State

See `PROJECT_STATE.md` for the live version — it's updated every session and this section is not. As of this writing (2026-07-31): the full v1 feature set plus v1.1 (Calendar History) is shipped and user-confirmed working; the backlog's "additional chart types" item shipped as the "By Day of Week" radial chart (implemented and unit-tested, simulator verification still pending); v1.2 (Widgets) was scoped and researched but is explicitly paused pending a paid Apple Developer account, at the user's request — do not raise it again until they say they're ready. Two open items worth remembering for a future session: `HabitsView`'s "+" button not responding to synthetic taps in this environment's simulator-automation tool (`KNOWN_ISSUES.md` R7) was worked around via direct user testing, not diagnosed, and may recur; and the radial chart itself hasn't been visually confirmed yet (`TODO.md`'s one open Medium item).
...
- **Physical-device deployment pipeline:** `xcrun xctrace list devices` (find UDID) → `xcodebuild build -destination 'id=<UDID>' -allowProvisioningUpdates` → `xcrun devicectl device install app --device <UDID> <path>.app` → `xcrun devicectl device process launch --device <UDID> <bundle-id>`. First install on any device needs a one-time manual trust step: Settings → General → VPN & Device Management → trust the developer certificate. **Caught 2026-07-30:** `xctrace` and `devicectl` report *different identifier strings for the same device* — use the `xctrace` one for `xcodebuild -destination`, the `devicectl` one (from `xcrun devicectl list devices`) for both `devicectl` subcommands, or get a misleading "device not found" error. A device listed as `connected (no DDI)` usually just needs "Trust This Computer?" confirmed on-device — try the build/install anyway rather than assuming it's broken.
- **Simulator-automation tap coordinates are in points, not screenshot pixels.** Confirmed 2026-07-30: tapping pixel-scale coordinates directly (treating a screenshot's rendered pixel dimensions as if they were the tool-reported point coordinate space, e.g. 402×874 on iPhone 16 Pro) silently misses controls or lands out of bounds — and looks identical to "the environment is flaky" or "taps stopped registering app-wide." This may explain some of an earlier session's reported tap-reliability issues. Always scale by the ratio between the screenshot and the `attach` call's reported point dimensions before tapping.
- **Before concluding a UI interaction is broken from simulator-automation tooling alone, verify on a physical device or with a human at the keyboard.** Two separate "known issues" (emoji entry, custom `ColorPicker`) turned out to be tooling limitations specific to a headless/streamed simulator (no on-screen software keyboard; `UIColorWell` not responding to synthetic taps) — not app defects — once actually tested on a real device. **Confirmed again 2026-07-31** with a third, more isolated case: `HabitsView`'s "+" toolbar button alone stopped responding to synthetic taps while every other control worked normally in the same session — resolved the same way, by the user testing directly, without ever diagnosing the tooling gap itself.
- **Plan Mode earns its cost on requests that bundle a safe feature with a risky one.** Asked for Calendar History and a WidgetKit widget together, researching first (rather than implementing immediately) surfaced that the widget's App Group requirement risked the user's free-tier code signing before any `project.yml` change was made. Surfacing that risk via `AskUserQuestion` let the user make an informed call (drop the widget, keep the safe feature) instead of discovering the risk mid-implementation or not at all.
- **A user's explicit "don't ask me about X again" is a standing instruction, not a one-turn preference** — worth writing to persistent memory (not just this file), since it needs to survive across separate conversations, not just the rest of one session.
...
Full prioritized list in `TODO.md`. Headline: the v1 feature set plus v1.1 (Calendar History) is complete and shipped. What's left is either explicitly paused (Widgets v1.2 — blocked on a paid Apple Developer account, at the user's own request; do not raise it proactively), version-gated (iCloud Sync v1.3, Apple Watch v1.4 — real scope jumps, not to be implemented ahead of schedule), or explicitly deferred pending more usage data (cross-habit correlation insights).
